#!/usr/bin/python3
# par github.com/BG47510
import requests
import sys

headers = {
    "User-Agent":"Mozilla/5.0 (X11; Linux x86_64; rv:121.0) Gecko/20100101 Firefox/121.0",
    "Referer":"http://api.pluto.tv",
    "visitorCountry":"CA",
    "Accept-Language":"en-EN,en;q=0.8"
}

def snif(pluto):
    erreur = "https://github.com/BG47510/Zap/raw/main/assets/error.m3u8"
    print('#EXTM3U url-tvg="https://github.com/matthuisman/i.mjh.nz/raw/refs/heads/master/PlutoTV/all.xml.gz" \n')
    try:
        api = requests.get(pluto, headers=headers).json()
    #print("#EXTM3U")
        for p in api:
              try:
                  print('')
                  idflux = (f'{p["_id"]}') # aussi tvg-id   tvg-ID=\"{name}\"
                  m3u8 = (f'https://stitcher.pluto.tv/v1/stitch/embed/hls/channel/{idflux}/master.m3u8?\
terminate=false&sid=WEB-45961ff4-d9cc-44b0-9a73-6e39ffde45ad\
&deviceDNT=0&deviceModel=web&deviceVersion=web&deviceId=web\
&appVersion=web&deviceType=web&deviceMake=web%7CReferer%3Dhttp%3A%2F%2Fapi.pluto.tv%2F%26User-Agent%3Dhttp-user-agent%3DMozilla%2F5.0+%28X11%3B+Linux+x86_64%3B+rv%3A121.0%29+Gecko%2F20100101+Firefox%2F121.0')
                  nom = (f',{p["name"]}')
                  groupe = (f'#EXTINF:-1 tvg-name="{p["name"]}" tvg-id="{p["_id"]}" group-title="{p["category"]}"')
                  print(groupe + nom)
              except Exception as e:
                  m3u8 = requests.get(erreur).text
              finally:
              	print(m3u8)
    except:
        m3u8 = requests.get(erreur).text
        print(m3u8)
clic = snif(sys.argv[1])
